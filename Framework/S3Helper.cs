...
public class S3Helper
{
    ...
    //public string UploadToS3(HttpPostedFileBase file, ResizeSettings setting, ImageResizerModel model, string filename = null)
    //{
    //    string isUploaded = null;
    //    if (file.ContentLength == 0) { return isUploaded; }
    //    if (model.id == 0) { return isUploaded; }
    ...
    //    if (filename == null)
    //    {
    //        filename = Hash(file.FileName, model.id);
    //    }
    //    var destination = String.Join("/", new String[] { env, model.country_code, model.folder, filename });

    //    setting.Format = Path.GetExtension(filename).Replace(".", "");
    //    var TempFile = TempPath + filename;
    //    ImageBuilder.Current.Build(file.InputStream, TempFile, setting);
    ...
    //        request.WithBucketName(bucket);
    //        request.WithKey(destination);
    //        request.AddHeaders(Amazon.S3.Util.AmazonS3Util.CreateHeaderEntry("Cache-Control", "max-age=31536000"));
    //        request.AddHeaders(Amazon.S3.Util.AmazonS3Util.CreateHeaderEntry("Expires", "Tue, 01 Jan 2030 03:54:42 GMT"));
    //        client.PutObject(request);

    //        SetACLRequest aclRequest = new SetACLRequest();
    //        aclRequest.Key = destination;
    //        aclRequest.BucketName = bucket;
    //        aclRequest.CannedACL = S3CannedACL.PublicRead;
    //        SetACLResponse aclResponse = client.SetACL(aclRequest);
    //        isUploaded = filename;
    ...
    //this process usually takes 10-15 min... wtf.. comment out portion to let it done in background
